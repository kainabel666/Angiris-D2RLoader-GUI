// ═══════════════════════════════════════════════════════════════════════
//  config_editor.cpp — see config_editor.h for the interface
// ═══════════════════════════════════════════════════════════════════════
...
bool ParseIniLine(const wstring& line, const wstring& key, wstring& value) {
    wstring s = line;
    TrimWs(s);
    // Skip blank lines, comment lines (both ; ini-style and # toml-style),
    // and section headers.
    if (s.empty() || s[0] == L';' || s[0] == L'#' || s[0] == L'[') return false;
    size_t eq = s.find(L'=');
...
// String-typed core. Returns `def` on missing section/key. The value
// is trimmed of surrounding whitespace before returning.
    ...
    while (lineStart <= contents.size()) {
        ...
        if (!trimmed.empty() && trimmed[0] == L'[') {
            ...
            if (ParseIniLine(line, key, val)) {
                return val;    // already TrimWs'd by ParseIniLine
            }
        }
...
int IniGetInt(const wstring& path, const wstring& section,
    ...
    wstring val = IniGetStr(path, section, key, L"");
...
// String-typed core. `newValue` is written as-is after the standard
// "key = " prefix — e.g. IniSetStr(..., L"true") produces
// "key = true". Callers wanting a leading-space bool style like the
// int variant already provides just pass "true"/"false".
void IniSetStr(const wstring& path, const wstring& section,
               const wstring& key, const wstring& newValue) {
    wstring contents = ReadTextFile(path);
    wstring wantSection = L"[" + section + L"]";
    const wstring& valStr = newValue;
...
void IniSetInt(const wstring& path, const wstring& section,
    ...
    IniSetStr(path, section, key, valBuf);
}
